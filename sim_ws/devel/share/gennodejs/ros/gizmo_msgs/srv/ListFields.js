...
let audio_msgs = _finder('audio_msgs');
...
class ListFieldsResponse {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.fields.forEach((val) => {
      bufferOffset = audio_msgs.msg.Field.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.fields[i] = audio_msgs.msg.Field.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.fields.forEach((val) => {
      length += audio_msgs.msg.Field.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    if (msg.fields !== undefined) {
      ...
      for (let i = 0; i < resolved.fields.length; ++i) {
        resolved.fields[i] = audio_msgs.msg.Field.Resolve(msg.fields[i]);
      }
    }
    else {
      resolved.fields = []
...
module.exports = {
  ...
  datatype() { return 'gizmo_msgs/ListFields'; }
};
